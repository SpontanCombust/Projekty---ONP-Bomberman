...
void explosion( LevelMap *level_map, int remaining_range, int tile_x, int tile_y, int x_step, int y_step, SFX * sfx_container[], int container_size, ALLEGRO_BITMAP *explosion_bmp )
{
    if( !areCoordsOutOfBounds( tile_x, tile_y, level_map->rows, level_map->collumns ) && remaining_range > 0 )
    {
        ...
        {
            addSFXToContainer( sfx_container, container_size, createSFX( pixelFromTile( tile_x ), pixelFromTile( tile_y ), EXPLOSION_LIFESPAN, explosion_bmp ) );
...
{
    addSFXToContainer( sfx_container, container_size, createSFX( pixelFromTile( bomb->tile_x ), pixelFromTile( bomb->tile_y ), EXPLOSION_LIFESPAN, explosion_bmp ) );
